...
namespace club.Services
{
    ...
    public class BookingService : IBookingService
    {
        ...
        public async Task<BookingResponse> UpdateAsync(int id, BookingSaveResource bookingSaveResource)
        {
            ...
            booking.DateTime = bookingSaveResource.DateTime;
        ...
        private async Task<BookingResponse> CommonValidations(BookingSaveResource bookingSaveResource)
        {
            ...
            if (bookingSaveResource.DateTime.Hour < 8 || bookingSaveResource.DateTime.Hour > 22)
                return new BookingResponse(400, "Bad booking time", "DateTime", "Bookings hour must be between 08:00 and 22:00");

            if (bookingSaveResource.DateTime.Minute != 0 || bookingSaveResource.DateTime.Second != 0)
                return new BookingResponse(400, "Bad booking time", "DateTime", "Bookings only accept full hours");

            if (bookingSaveResource.DateTime < DateTime.Today)
                return new BookingResponse(400, "Bad booking time", "DateTime", "DateTime is older than today");
            
            DateTime dayStartDate = new DateTime(bookingSaveResource.DateTime.Year, bookingSaveResource.DateTime.Month, bookingSaveResource.DateTime.Day, 8, 0, 0);
            DateTime dayEndDate = new DateTime(bookingSaveResource.DateTime.Year, bookingSaveResource.DateTime.Month, bookingSaveResource.DateTime.Day, 22, 0, 0);

            var bookingsOfCourt = await _bookingRepository.GetBookings(bookingSaveResource.DateTime, bookingSaveResource.DateTime, 0, bookingSaveResource.CourtId);

            if (bookingsOfCourt.Count > 0)
                return new BookingResponse(400, "Court already booked", "CourtId", "Court is already booked for " + bookingSaveResource.DateTime.ToString("yyyy-MM-dd HH:mm"));

            var bookingsOfMember = await _bookingRepository.GetBookings(dayStartDate, dayEndDate, bookingSaveResource.MemberId);
            ...
            var bookingsOfMemberHour = await _bookingRepository.GetBookings(bookingSaveResource.DateTime, bookingSaveResource.DateTime, bookingSaveResource.MemberId);
